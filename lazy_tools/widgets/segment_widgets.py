...
class SegmentationWorker(QThread):
    ...
    def __init__(self, image_path: str, prompt: str, output_path: str, output_type: str = 'overlay'):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            cmd = [VENV_PYTHON_PATH, LAZY_SEGMENT_SCRIPT_PATH, self.image_path, self.prompt, self.output_path]
            ...
            if return_code == 0:
                ...
                if os.path.exists(self.output_path):
                    ...
                    self.error.emit("Segmentation completed but no output file was generated")
            else:
                self.error.emit(f"Segmentation process failed with return code: {return_code}")
...
class SegmentSection(QWidget):
    ...
    def setup_ui(self):
        ...
        self.overlay_radio.setChecked(DEFAULT_OUTPUT_TYPE == 'overlay')  # Use config default
        self.overlay_radio.setToolTip("Show segmented areas with red overlay on original image")
        ...
        self.cutout_radio.setChecked(DEFAULT_OUTPUT_TYPE == 'cutout')  # Use config default
        self.cutout_radio.setToolTip("Create cutout with transparent background (perfect for layers)")
    ...
    def run_segmentation(self):
        ...
        try:
            ...
            if not prompt:
                QMessageBox.warning(self, "No Prompt", "Please enter what object to find.")
                return
            ...
            try:
                ...
                if not rgb_image.save(input_path, "PNG"):
                    QMessageBox.critical(self, "Export Error", "Failed to save document as PNG.")
                    return
                    
            except Exception as export_error:
                QMessageBox.critical(self, "Export Error", f"Failed to export document: {str(export_error)}")
                return
            
            # Start segmentation in worker thread
            self.start_processing()
            
            self.worker_thread = SegmentationWorker(input_path, prompt, output_path, output_type)
            ...
            QMessageBox.critical(self, "Error", f"Failed to start segmentation: {str(e)}")
    ...
    def on_segmentation_finished(self, output_path: str):
        ...
        try:
            ...
            prompt = self.prompt_input.text().strip()
            output_type = 'cutout' if self.cutout_radio.isChecked() else 'overlay'
            layer_name = f"AI Segment ({output_type.title()}): {prompt}"
            ...
            QMessageBox.critical(self, "Layer Error", f"Failed to add segmented layer: {str(e)}")
